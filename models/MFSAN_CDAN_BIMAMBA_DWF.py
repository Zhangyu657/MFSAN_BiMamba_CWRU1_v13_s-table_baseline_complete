# -*- coding: utf-8 -*-
"""
MFSAN-CDAN-BiMamba-DWF

在 MFSAN-CDAN-BiMamba 的基础上，新增 Dynamic Weighted Fusion (DWF)
动态源域加权融合机制。

核心改进：
1. 保留当前 BiMamba-SmallGate 主模型：
   - MSCNN 主分支；
   - BiMamba-Attention 辅助分支；
   - SmallGate 小残差融合；
   - MFSAN 多源迁移；
   - CDAN 条件域对抗；
   - CDA 条件 MMD；
...
2. 新增 DWF 动态源域加权融合：
   原始 MFSAN 测试阶段通常使用简单平均：
       p = (p1 + p2 + p3) / 3

   DWF 改为根据每个源分类器对当前目标样本的预测熵进行动态加权：
       wi = softmax(-H(pi) / tau)
       p  = sum_i wi * pi

   直观含义：
       对当前目标样本越自信的源域分类器，融合权重越高；
       越不确定的源域分类器，融合权重越低。

3. 训练阶段目标域熵最小化也使用 DWF 后的 fused probability，
   保持训练和测试融合策略一致。
"""
...
class Trainer(TrainerBase):
    ...
    def __init__(self, args):
        ...
        if args.train_mode != 'multi_source':
            raise ValueError('MFSAN_CDAN_BIMAMBA_DWF is designed for --train_mode multi_source.')
        ...
        # ========== Backbone: MSCNN + BiMamba-Attention small-gate fusion ==========
        # 为了减少 opt.py 改动：--backbone CNN 时也使用增强 backbone。
        ...
        logging.info('Using model: MFSAN_CDAN_BIMAMBA_DWF')
        ...
        if hasattr(self.G, 'get_gate'):
            logging.info('Initial BiMamba-Att residual gate: {:.6f}'.format(
                self.G.get_gate().detach().item()
            ))
            if hasattr(self.G, 'gate_max'):
                logging.info('Max BiMamba-Att residual gate: {:.6f}'.format(
                    self.G.gate_max
                ))
        ...
        self.dwf_tau = float(getattr(args, 'dwf_tau', 0.5))
        self.dwf_tau = max(self.dwf_tau, 1e-6)
        self.dwf_detach_weights = bool(getattr(args, 'dwf_detach_weights', True))

        self.entropy_eps = 1e-5

        logging.info('MFSAN-CDAN-BiMamba-DWF lambda_adv: {:.6f}'.format(self.lambda_adv))
        logging.info('MFSAN-CDAN-BiMamba-DWF lambda_grl: {:.6f}'.format(self.lambda_grl))
        logging.info('MFSAN-CDAN-BiMamba-DWF adv_detach_prob: {}'.format(self.adv_detach_prob))
        logging.info('MFSAN-CDAN-BiMamba-DWF adv_use_entropy_weight: {}'.format(self.adv_use_entropy_weight))
        logging.info('MFSAN-CDAN-BiMamba-DWF adv_conf_thresh: {:.6f}'.format(self.adv_conf_thresh))
        logging.info('MFSAN-CDAN-BiMamba-DWF lambda_cda: {:.6f}'.format(self.lambda_cda))
        logging.info('MFSAN-CDAN-BiMamba-DWF lambda_ent: {:.6f}'.format(self.lambda_ent))
        logging.info('MFSAN-CDAN-BiMamba-DWF DWF tau: {:.6f}'.format(self.dwf_tau))
        logging.info('MFSAN-CDAN-BiMamba-DWF DWF detach weights: {}'.format(self.dwf_detach_weights))
        logging.info('MFSAN-CDAN-BiMamba-DWF joint feature dim: {} x {} = {}'.format(
    ...
    def save_model(self):
        torch.save({
            ...
            'dwf_tau': self.dwf_tau,
            'dwf_detach_weights': self.dwf_detach_weights,
    ...
    def load_model(self):
        ...
        if 'dwf_tau' in ckpt:
            self.dwf_tau = float(ckpt['dwf_tau'])
            logging.info('Loaded DWF tau from checkpoint: {:.6f}'.format(self.dwf_tau))
        if 'dwf_detach_weights' in ckpt:
            self.dwf_detach_weights = bool(ckpt['dwf_detach_weights'])
            logging.info('Loaded DWF detach weights from checkpoint: {}'.format(self.dwf_detach_weights))
    ...
    def _dynamic_weighted_fusion(self, probs_list):
        """
        Dynamic Weighted Fusion for multi-source classifiers.

        Args:
            probs_list: list of length num_source.
                Each element has shape [B, C], already after softmax.

        Returns:
            fused_prob: [B, C]
            weights: [num_source, B]
                Per-source dynamic weights for each target sample.
        """
        probs_stack = torch.stack(probs_list, dim=0)  # [S, B, C]
        probs_safe = torch.clamp(probs_stack, min=self.entropy_eps, max=1.0)

        # entropy: [S, B]
        entropy = -(probs_safe * torch.log(probs_safe)).sum(dim=2)

        # lower entropy => higher reliability
        weights = torch.softmax(-entropy / self.dwf_tau, dim=0)  # [S, B]

        # 为了稳定训练，默认 detach 权重，让 DWF 只负责可靠性融合，
        # 不让模型通过“操纵权重”来绕开目标域熵约束。
        if self.dwf_detach_weights:
            weights_for_fusion = weights.detach()
        else:
            weights_for_fusion = weights

        fused_prob = (weights_for_fusion.unsqueeze(2) * probs_stack).sum(dim=0)
    ...
    def _train_one_epoch(self, epoch_acc, epoch_loss):
        dwf_weight_sum = torch.zeros(self.num_source, device=self.device)
        dwf_weight_count = 0
        ...
        for i in tqdm(range(self.num_iter), ascii=True):
            ...
            # Target entropy minimization with DWF
            probs_t_fused, dwf_weights = self._dynamic_weighted_fusion(probs_t_all)
            loss_ent = self._target_entropy(probs_t_fused)

            with torch.no_grad():
                dwf_weight_sum += dwf_weights.mean(dim=1)
                dwf_weight_count += 1
        ...
        if hasattr(self.G, 'get_gate'):
            logging.info('BiMamba-Att residual gate: {:.6f}'.format(
                self.G.get_gate().detach().item()
            ))
            if hasattr(self.G, 'gate_max'):
                logging.info('Max BiMamba-Att residual gate: {:.6f}'.format(
        ...
        if dwf_weight_count > 0:
            avg_w = (dwf_weight_sum / dwf_weight_count).detach().cpu().tolist()
            logging.info('DWF average source weights: {}'.format(
                ', '.join(['src{}={:.4f}'.format(i, w) for i, w in enumerate(avg_w)])
            ))

        logging.info(
            'MFSAN-CDAN-BiMamba-DWF active: lambda_adv={:.6f}, lambda_grl={:.6f}, lambda_cda={:.6f}, lambda_ent={:.6f}, dwf_tau={:.6f}, adv_detach_prob={}, entropy_weight={}'.format(
                self.lambda_adv,
                self.lambda_grl,
                self.lambda_cda,
                self.lambda_ent,
    ...
    def _eval(self, data, actual_labels, correct, total):
        ...
        fused_prob, dwf_weights = self._dynamic_weighted_fusion(probs_tgt)

        # 给 plot/test 时额外保存 DWF 权重，后续可以画源域权重分布图
        if hasattr(self, '_eval_dwf_weight_list'):
            self._eval_dwf_weight_list.append(dwf_weights.detach().cpu())
