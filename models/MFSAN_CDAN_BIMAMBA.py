...
在 MFSAN-CDAN-BLA 的基础上，将 BiLSTM-Attention 辅助分支替换为
...
2. 共享特征提取器改为：
   MSCNN 主分支 + BiMamba-Attention 辅助分支 + Small-Gate 融合：
...
class Trainer(TrainerBase):
    ...
    def __init__(self, args):
        ...
        if args.train_mode != 'multi_source':
            raise ValueError('MFSAN_CDAN_BIMAMBA is designed for --train_mode multi_source.')
        ...
        logging.info('Using model: MFSAN_CDAN_BIMAMBA')
        ...
        logging.info('MFSAN-CDAN-BiMamba lambda_adv: {:.6f}'.format(self.lambda_adv))
        logging.info('MFSAN-CDAN-BiMamba lambda_grl: {:.6f}'.format(self.lambda_grl))
        logging.info('MFSAN-CDAN-BiMamba adv_detach_prob: {}'.format(self.adv_detach_prob))
        logging.info('MFSAN-CDAN-BiMamba adv_use_entropy_weight: {}'.format(self.adv_use_entropy_weight))
        logging.info('MFSAN-CDAN-BiMamba adv_conf_thresh: {:.6f}'.format(self.adv_conf_thresh))
        logging.info('MFSAN-CDAN-BiMamba lambda_cda: {:.6f}'.format(self.lambda_cda))
        logging.info('MFSAN-CDAN-BiMamba lambda_ent: {:.6f}'.format(self.lambda_ent))
        logging.info('MFSAN-CDAN-BiMamba joint feature dim: {} x {} = {}'.format(
    ...
    def _train_one_epoch(self, epoch_acc, epoch_loss):
        ...
        logging.info(
            'MFSAN-CDAN-BiMamba active: lambda_adv={:.6f}, lambda_grl={:.6f}, lambda_cda={:.6f}, lambda_ent={:.6f}, adv_detach_prob={}, entropy_weight={}'.format(
                self.lambda_adv,
                self.lambda_grl,
                self.lambda_cda,
                self.lambda_ent,
