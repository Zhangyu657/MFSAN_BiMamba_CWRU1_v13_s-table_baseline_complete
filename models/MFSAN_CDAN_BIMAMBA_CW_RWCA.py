# -*- coding: utf-8 -*-
"""
MFSAN-CDAN-BiMamba-CW-RWCA

CW-RWCA = Class-wise Reliability-Weighted Class Alignment

在原 MFSAN-CDAN-BiMamba-RWCA 基础上，把“每个源域一个全局权重 w_s”升级为
“每个类别、每个源域一个权重 w_{s,c}”。这样可以针对 PU_0 目标域中 KA04/KA16
这类迁移困难类别，单独选择更可靠的源域，缓解类别级负迁移。

推荐模型名：MFSAN_CDAN_BIMAMBA_CW_RWCA
"""
...
class Trainer(TrainerBase):
    ...
    def __init__(self, args):
        ...
        if args.train_mode != 'multi_source':
            raise ValueError('MFSAN_CDAN_BIMAMBA_CW_RWCA is designed for --train_mode multi_source.')
        ...
        logging.info('Using model: MFSAN_CDAN_BIMAMBA_CW_RWCA')
        ...
        logging.info('MFSAN-CDAN-BiMamba-CW-RWCA lambda_adv: {:.6f}'.format(self.lambda_adv))
        logging.info('MFSAN-CDAN-BiMamba-CW-RWCA lambda_grl: {:.6f}'.format(self.lambda_grl))
        logging.info('MFSAN-CDAN-BiMamba-CW-RWCA lambda_cda: {:.6f}'.format(self.lambda_cda))
        logging.info('MFSAN-CDAN-BiMamba-CW-RWCA lambda_ent: {:.6f}'.format(self.lambda_ent))
        logging.info('MFSAN-CDAN-BiMamba-CW-RWCA lambda_clmmd: {:.6f}'.format(self.lambda_clmmd))
        logging.info('MFSAN-CDAN-BiMamba-CW-RWCA rw_tau: {:.6f}'.format(self.rw_tau))
        logging.info('MFSAN-CDAN-BiMamba-CW-RWCA rw_mmd_weight: {:.6f}'.format(self.rw_mmd_weight))
        logging.info('MFSAN-CDAN-BiMamba-CW-RWCA rw_ent_weight: {:.6f}'.format(self.rw_ent_weight))
        logging.info('MFSAN-CDAN-BiMamba-CW-RWCA rw_detach_weights: {}'.format(self.rw_detach_weights))
        logging.info('MFSAN-CDAN-BiMamba-CW-RWCA joint feature dim: {} x {} = {}'.format(
    ...
    def _train_one_epoch(self, epoch_acc, epoch_loss):
        ...
        for _ in tqdm(range(self.num_iter), ascii=True):
            ...
            # CW-RWCA: class-wise source reliability weights [K, C]
            class_src_weights = self._class_source_reliability_weights(
        ...
        avg_class_weights = (class_weight_sum / max(float(self.num_iter), 1.0)).detach().cpu().numpy()
        ema_class_weights = self.class_source_weight_ema.detach().cpu().numpy()

        logging.info('CW-RWCA average train source weights: {}'.format(
            ', '.join(['src{}={:.4f}'.format(i, avg_weights[i]) for i in range(self.num_source)])
        ))
        logging.info('CW-RWCA EMA source weights: {}'.format(
            ', '.join(['src{}={:.4f}'.format(i, ema_weights[i]) for i in range(self.num_source)])
        ))
        for c in self._cw_log_classes:
            logging.info('CW-RWCA average train class-{} source weights: {}'.format(
                c, ', '.join(['src{}={:.4f}'.format(i, avg_class_weights[i, c]) for i in range(self.num_source)])
            ))
            logging.info('CW-RWCA EMA class-{} source weights: {}'.format(
        ...
        logging.info(
            'MFSAN-CDAN-BiMamba-CW-RWCA active: lambda_adv={:.6f}, lambda_grl={:.6f}, lambda_cda={:.6f}, lambda_clmmd={:.6f}, lambda_ent={:.6f}'.format(
