...
4. 根据距离计算 alpha_dynamic = softmax(-normalized_mmd_i / tau)；
5. 将动态权重与平均权重混合，避免训练早期权重过度偏置；
6. 设置最小源域权重，避免某个源域被压得太低；
7. 分类损失不加权，只对 MMD 域对齐损失加权，保留完整源域分类监督。
'''
...
class Trainer(TrainerBase):
    ...
    def __init__(self, args):
        ...
        # 保留原始 MFSAN 的 MK-MMD，保证和原始 MFSAN 公平对比
        ...
        # 源域权重下限，避免某个源域权重过低
        # 3 个源域时，0.15 表示任何源域最低保留 15% 权重
        self.min_source_weight = 0.15

        # 权重尺度保持：
        # alpha_i * num_source，保证均匀权重时 MMD 损失等价原始 MFSAN
    ...
    def _compute_dynamic_weights(self):
        ...
        2. 与平均权重 uniform_alpha 混合，避免训练早期过度偏置；
        3. 加入最小权重保护，避免某个源域被压得太低。
    ...
    def _train_one_epoch(self, epoch_acc, epoch_loss):
        ...
        for i in tqdm(range(self.num_iter), ascii=True):
            ...
            if self.keep_loss_scale:
                # 如果 alpha=[1/3,1/3,1/3]，cur_weight=1，等价原始 MFSAN 的 MMD 权重
                cur_weight = cur_alpha * self.num_source
            else:
                cur_weight = cur_alpha

            # Conservative dynamic weighted loss
            # 关键改动：
            # 1. 分类损失 loss_c 不加权，保留完整源域分类监督；
            # 2. 只对 MMD 域对齐损失加动态权重；
            # 3. L1 分类器一致性保持原始 MFSAN 逻辑。
            loss = (
                loss_c
            ...
            # 原始 loss，用于和 MFSAN baseline 对比
            epoch_loss['Source Classifier'] += loss_c
            epoch_loss['MMD'] += loss_mmd
            epoch_loss['L1'] += loss_l1

            # 新增日志：分类损失未加权，MMD 加权
            epoch_loss['Unweighted Source Classifier'] += loss_c.detach()
    ...
    def _eval(self, data, actual_labels, correct, total):
        ...
        # 测试阶段仍然保持原始 MFSAN 的求和融合，保证稳定和公平
