...
Based on original MFSAN.py from TL-Fault-Diagnosis-Library.

改进点：
1. 保留原始 MFSAN 的训练循环、MMD 实现、L1 一致性、测试融合、保存/加载逻辑；
2. 增加动态源域权重 Dynamic Source Weight；
3. 使用每个源域与目标域的 MMD 运行均值估计源-目标距离；
4. 根据距离计算 alpha_i = softmax(-normalized_mmd_i / tau)；
5. 使用 alpha_i 加权当前源域的分类损失和 MMD 损失；
6. 使用 alpha_i * num_source 做尺度保持，保证权重均匀时等价于原始 MFSAN。
'''
...
class Trainer(TrainerBase):
    ...
    def __init__(self, args):
        ...
        if args.train_mode != 'multi_source':
            raise ValueError('DW_MFSAN is designed for --train_mode multi_source.')
        ...
        # 保留原始 MFSAN 的 MK-MMD，不再自己重写 MMD，保证公平对比
        ...
        # 保留原始 MFSAN 的迭代次数逻辑，避免变成 814 steps 那种不公平情况
        self.num_iter = sum([len(self.dataloaders[s]) for s in self.src])

        # ========== Dynamic Source Weight settings ==========
        # MMD EMA：用于估计每个源域和目标域的平均距离
        self.mmd_ema = torch.ones(self.num_source, device=self.device)

        # EMA 动量。越大越平滑，越小越敏感
        self.mmd_ema_momentum = 0.90

        # softmax 温度。越小权重越尖锐，越大越平均
        self.dw_tau = 1.0

        # 权重尺度保持。alpha_i * num_source，保证均匀权重时等价原始 MFSAN
    ...
    def _compute_dynamic_weights(self):
        ...
        先标准化 MMD，避免数值尺度太小导致 softmax 后几乎全是 1/3。
        alpha_i = softmax(- normalized_mmd_i / tau)

        MMD 越小，说明源域越接近目标域，权重越大。
        """
        d = self.mmd_ema.detach()

        d_std = torch.std(d, unbiased=False)
        if d_std.item() < 1e-8:
            alpha = torch.ones_like(d) / len(d)
            return alpha

        d_norm = (d - d.mean()) / (d_std + 1e-8)
        alpha = torch.softmax(-d_norm / self.dw_tau, dim=0)
    ...
    def _train_one_epoch(self, epoch_acc, epoch_loss):
        ...
        for i in tqdm(range(self.num_iter), ascii=True):
            ...
            # 保留原始 MFSAN 的写法，不改成 pair_count，保证和 baseline 一致
            loss_l1 /= self.num_source

            # update dynamic source weight
            ...
            if self.keep_loss_scale:
                # 关键：尺度保持
                # 如果 alpha=[1/3,1/3,1/3]，cur_weight = 1.0，完全等价原始 MFSAN
            ...
            # 为了和原始 MFSAN 对比，仍然记录原始 loss 值
            epoch_loss['Source Classifier'] += loss_c
            epoch_loss['MMD'] += loss_mmd
            epoch_loss['L1'] += loss_l1

            # 额外记录动态加权后的分类和 MMD，方便分析
            epoch_loss['DW Source Classifier'] += cur_weight.detach() * loss_c.detach()
            ...
            # epoch-level weight and mmd statistics
        ...
        # log source-target MMD running status
        avg_mmd_list = []
        for k in range(self.num_source):
    ...
    def _eval(self, data, actual_labels, correct, total):
        ...
        # 第一版测试阶段仍然保持原始 MFSAN 的平均/求和融合，保证稳定和公平
